  ...
  let rec loop () =
    ...
    | Some { Frame.meta; _ } ->
      ...
      let response =
        ...
        | Msg.Eval { source; autorun; check; cost } ->
          Eval.eval cap ~autorun ~check ~cost source
        ...
        | Msg.Markers { disarm; arm; disarm_sites; arm_sites; restore } ->
          Eval.markers ~disarm ~arm ~disarm_sites ~arm_sites ~restore
