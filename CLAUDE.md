...
session and speaks MCP over stdio. Nineteen tools: `eval`, `describe`, `require`,
...
session in a file's context; merlin, no session needed),
`signature` (an installed package's interfaces, no session needed), and `help`
(each tool's manual, behind a description kept short: ticket 062). There is
